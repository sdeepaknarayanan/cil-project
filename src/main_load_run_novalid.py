...
f.write(f"Model will be loaded from Location: {args.load_path}\n")
f.flush()
...
if args.model == 'resnet':
    ...
    print("Incorrect Argument for Model. Exiting")
    sys.exit(0)
...
for epoch in range(args.epochs):
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        optimizer.step()
        model.eval()
        train_acc+=metric_fn(output, target.float().cuda(), is_sig)
        epoch_loss+=loss.item()

    print(f"Loss for Epoch: {epoch} is {epoch_loss / (i + 1)} Accuracy is: {train_acc/(i+1)}")

    f.write(f"Loss for Epoch: {epoch} is {epoch_loss / (i + 1)} Accuracy is: {train_acc/(i+1)}\n")
    f.flush()

    if epoch % 10 == 0:
        torch.save(model.state_dict(), save_path+f"/model_epoch_regular_save_{epoch}")
