...
class UpBlockForUNetWithResNet50(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, up_conv_in_channels=None, up_conv_out_channels=None,
                 upsampling_method="conv_transpose", flag=None):
        super().__init__()
        self.flag = flag
...
class build_resnet(nn.Module):
    ...
    def __init__(self, n_classes=1, pretrained=True):
        ...
        up_blocks.append(UpBlockForUNetWithResNet50(2048, 1024, flag = "odd"))
    ...
    def forward(self, x, with_output_feature_map=False):
        ...
        for i, block in enumerate(self.down_blocks, 2):
            x = block(x)
            if i == (build_resnet.DEPTH - 1):
                continue
            pre_pools[f"layer_{i}"] = x
        x = self.bridge(x)

        for i, block in enumerate(self.up_blocks, 1):
            key = f"layer_{build_resnet.DEPTH - 1 - i}"
